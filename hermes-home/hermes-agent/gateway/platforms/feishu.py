...
class FeishuGroupRule:
    ...
    output_mode: str = "full"  # "full" | "minimal" - minimal suppresses intermediate status msgs
...
class FeishuAdapter(BasePlatformAdapter):
        ...
        if isinstance(raw_group_rules, dict):
            for chat_id, rule_cfg in raw_group_rules.items():
                ...
                group_rules[str(chat_id)] = FeishuGroupRule(
                    ...
                    output_mode=str(rule_cfg.get("output_mode", "full")).strip().lower(),
                )
    ...
    def _build_event_handler(self) -> Any:
        ...
        return (
            ...
            .register_p2_im_chat_access_event_bot_p2p_chat_entered_v1(
                lambda data: logger.debug("[Feishu] bot p2p chat entered event (no-op)")
            )
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        # PP_PERMISSION_V3: Handle approval card button clicks
        _pp_action_value = action_value.get("pp_action") if isinstance(action_value, dict) else None
        if _pp_action_value:
            return _pp_handle_card_action_v3(self, event=event, action_value=action_value, loop=loop)
        # PP_PERMISSION_V3 END
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        # PP_ADMIN_ONLY_APPROVAL_FIX: Pass open_id for admin verification
        self._submit_on_loop(loop, self._resolve_approval(approval_id, choice, user_name, open_id))
    ...
    async def _resolve_approval(self, approval_id: Any, choice: str, user_name: str, open_id: str = "") -> None:
        """Pop approval state and unblock the waiting agent thread.

        PP_ADMIN_ONLY_APPROVAL_FIX: Only admin users can approve/deny dangerous commands.
        Non-admin users receive "PERMISSION DENIED" and their choice is rejected.
        """
        # Admin verification - only admin open_ids can approve/deny
        if not _pp_is_admin_v3(open_id):
            logger.error(
                "[PP_ADMIN_ONLY] PERMISSION DENIED: Non-admin %s (%s) attempted to %s approval_id=%s",
                user_name, open_id, choice, approval_id
            )
            # Send permission denied message back to the user
            try:
                from tools.approval import resolve_gateway_approval
                # Deny the approval with "deny" choice to block the command
                count = resolve_gateway_approval(approval_id, "deny")
                logger.info("[PP_ADMIN_ONLY] Approval %s auto-denied for non-admin", approval_id)
            except Exception as exc:
                logger.error("Failed to auto-deny non-admin approval: %s", exc)
            return
        ...
        try:
            ...
            logger.info(
                "[PP_ADMIN_ONLY] Admin %s (%s) resolved %d approval(s) for session %s (choice=%s)",
                user_name, open_id, count, state["session_key"], choice,
            )
        except Exception as exc:
    ...
    async def _process_inbound_message(
        ...
        *,
        ...
        # PP_PERMISSION_V3: Check dangerous commands with card approval
        try:
            if os.getenv("FEISHU_PERMISSION_PROXY", "") == "1":
                # 获取发送者信息
                sender_open_id = str(getattr(sender_id, "open_id", "") or "") if sender_id else ""
                sender_user_id = str(getattr(sender_id, "user_id", "") or "") if sender_id else ""
                ...
                try:
                    ...
                    if event:
                        sender = getattr(event, "sender", None)
                        if sender:
                            sender_name = str(getattr(sender, "sender_name", "Unknown") or "Unknown")
                except Exception:
                    pass
                ...
                if msg_content:
                    try:
                        ...
                        raw = parsed.get("text", "") or parsed.get("content", "")
                        # PP_CONTENT_LIST_FIX: content can be a nested list (rich content like @tags)
                                ...
                                for block in blocks:
                                    if isinstance(block, list):
                                        ...
                                        if block.get("tag") == "text":
                                            ...
                                            parts.append("@" + block.get("user_name", ""))
                                return "".join(parts)
                ...
                # 调试日志：显示提取的内容
                logger.warning(f"[PP_V3_DEBUG] msg_content raw: {repr(msg_content)[:200]}")
                logger.warning(f"[PP_V3_DEBUG] content_text extracted: {repr(content_text)[:200]}")

                # 检查权限
                is_admin = _pp_is_admin_v3(sender_open_id)
                is_dangerous, danger_type = _pp_is_dangerous_v3(content_text)

                logger.warning(f"[PP_V3] User={sender_open_id} admin={is_admin} dangerous={is_dangerous} content_len={len(content_text)}")

                # 获取 chat_id
                chat_id = str(getattr(message, "chat_id", "") or sender_open_id) or sender_open_id

                if is_admin:
                    _pp_log_v3("ADMIN", sender_open_id, sender_name, content_text, "ALLOWED")
                elif is_dangerous:
                    # 非 admin + 危险命令 → 创建审批请求
                    ...
                    _pp_add_pending_v3(approval_id, pending_data)
                    _pp_log_v3("DANGEROUS", sender_open_id, sender_name, content_text, "PENDING", f"id={approval_id}")

                    # 获取 admin ID 并发送审批卡片
                    admin_ids = set()
                    env_admins = os.getenv("HERMES_ADMIN_OPEN_IDS", "")
                    if env_admins:
                        admin_ids.update(uid.strip() for uid in env_admins.split(",") if uid.strip())
                    ...
                    if config_path.exists():
                        try:
                            ...
                            for uid in extra.get("admins", []):
                                stripped = str(uid).strip()
                                if stripped:
                                    admin_ids.add(stripped)
                        except Exception:
                            pass

                    admin_id = list(admin_ids)[0] if admin_ids else None
                    if admin_id:
                        card_content = _pp_build_approval_card_v3(
                            approval_id, sender_name, sender_open_id, content_text, danger_type
                        )
                        try:
                            ...
                            if loop:
                                ...
                                asyncio.ensure_future(
                                    self._feishu_send_with_retry(
                                        chat_id=admin_id,
                                        msg_type="interactive",
                                        payload=json.dumps(card_content, ensure_ascii=False),
                                        reply_to=None,
                                        metadata=None
                                    ),
                                    loop=loop
                            ...
                            logger.warning(f"[PP_V3] Send card failed: {e}")

                    # 通知用户命令待审批
                    _pp_notify_user_v3(self, chat_id, False, content_text, "System")
                    logger.warning(f"[PP_V3] Dangerous command pending approval from {sender_open_id}, id={approval_id}")
                    return
                else:
                    _pp_log_v3("NORMAL", sender_open_id, sender_name, content_text, "ALLOWED")
        except Exception as e:
            logger.error(f"[PP_V3] Check error: {e}")
        # PP_PERMISSION_V3 END
    ...
    def is_group_minimal_mode(self, chat_id: str) -> bool:
        """Check if a group is configured for minimal output mode (no intermediate status msgs)."""
        if not chat_id:
            return False
        rule = self._group_rules.get(chat_id)
        if rule and hasattr(rule, 'output_mode'):
            return rule.output_mode == 'minimal'
        return False
...
# =============================================================================
# PP_PERMISSION_V2: Hermes Permission Proxy (插入版)
# =============================================================================
import os
import re
import json
...
def _pp_log(event_type, user_id, user_name, content, action, details=""):
    """写审计日志"""
    try:
        audit_file = Path.home() / ".hermes" / "logs" / "permission_audit.log"
...
PP_DANGEROUS_PATTERNS = [
    (r"\brem\s+", "delete"),
    (r"\bdel\s+", "delete"),
    (r"\brmdir\b", "delete"),
    (r"\brm\s+-rf", "recursive delete"),
    (r"\brm\s+-[^\s]*r", "recursive delete"),
    (r"\bchmod\b.*777", "777 permissions"),
    (r"\bchmod\b.*666", "666 permissions"),
    (r"\bchmod\b.*o\+w", "other writable"),
    (r"\bchown\b.*root", "chown to root"),
    (r"\bmkfs\b", "format filesystem"),
    (r"\bfdisk\b", "partition"),
    (r"\bparted\b", "partition"),
    (r"\bpkill\b", "kill process"),
    ...
    (r"\bhalt\b", "halt"),
    (r"\bnc\s+-[el]\b", "netcat"),
    (r"\bnetcat\b.*-e", "netcat"),
    (r"\bwget\b.*\|.*sh", "remote exec"),
    (r"\bcurl\b.*\|.*sh", "remote exec"),
    (r"\bDROP\s+(TABLE|DATABASE)\b", "sql drop"),
    (r"\bDELETE\s+FROM\b(?!.*\bWHERE\b)", "sql delete no where"),
    (r"\bTRUNCATE\s+", "truncate table"),
    (r">[>]?\s*/etc/", "write system dir"),
    (r">[>]?\s*/var/", "write system dir"),
    (r">[>]?\s*/root/", "write root dir"),
    (r"\bsudo\b", "sudo"),
    (r"\bsu\b\s+", "switch user"),
    (r"\bsystemctl\b.*(stop|restart|disable)", "systemctl"),
    (r"\bdocker\b.*(rm|rmi|prune)", "docker"),
]

def _pp_is_dangerous(text):
    """检测危险命令"""
    if not text:
        return False, ""
    for pattern, desc in PP_DANGEROUS_PATTERNS:
        if re.search(pattern, text, re.IGNORECASE):
            return True, desc
    return False, ""

def _pp_check_and_forward(self, sender_id, data, message):
    """检查权限并转发危险命令"""
    try:
        if os.getenv("FEISHU_PERMISSION_PROXY", "") != "1":
            return  # Disabled
        ...
        if msg_content:
            try:
                parsed = json.loads(msg_content)
                content_text = parsed.get("text", "") or parsed.get("content", "")
            except Exception:
                content_text = str(msg_content)

        # 检查权限
        is_admin = _pp_is_admin(sender_open_id)
        is_dangerous, danger_type = _pp_is_dangerous(content_text)

        logger.info(f"[PP] User={sender_open_id} admin={is_admin} dangerous={is_dangerous}")

        if is_admin:
            _pp_log("ADMIN", sender_open_id, sender_name, content_text, "ALLOWED")
        elif is_dangerous:
            _pp_log("DANGEROUS", sender_open_id, sender_name, content_text, "BLOCKED", f"type={danger_type}")
            ...
            if admin_id:
                forward_msg = (
                    "\u26a0\ufe0f **危险命令已阻止**\n\n"
                    f"**发送者**: {sender_name}\n"
                    f"**OpenID**: `{sender_open_id}`\n"
                    f"**危险类型**: {danger_type}\n\n"
                    f"**命令内容**:\n```\n{content_text[:500]}\n```\n\n"
                    "该命令被阻止。如需执行，请管理员手动处理。"
                )
                try:
                    loop = getattr(self, "_loop", None)
                    if loop:
                        import asyncio
                        asyncio.ensure_future(
                            self.send_message(chat_id=admin_id, text=forward_msg),
                            loop=loop
                        )
                except Exception as e:
                    logger.warning(f"[PP] Forward to admin failed: {e}")

            logger.warning(f"[PP] Dangerous command blocked from {sender_open_id}: {content_text[:50]}...")
            return True  # 表示已阻止
        else:
            _pp_log("NORMAL", sender_open_id, sender_name, content_text, "ALLOWED")
    except Exception as e:
        logger.error(f"[PP] Permission check error: {e}")
    return False  # 表示未阻止


# =============================================================================
# PP_PERMISSION_V3: Hermes Permission Proxy (Interactive Card Approval)
# =============================================================================

def _pp_is_admin_v3(open_id):
    """检查是否为管理员"""
    if not open_id:
        return False
    admin_ids = set()
...
def _pp_log_v3(event_type, user_id, user_name, content_text, action, details=""):
    """写审计日志"""
    try:
        audit_file = Path.home() / ".hermes" / "logs" / "permission_audit.log"
        audit_file.parent.mkdir(parents=True, exist_ok=True)
...
# PP_CHINESE_PATTERNS: Added Chinese dangerous command detection
PP_DANGEROUS_PATTERNS_V3 = [
    (r"\brem\s+", "delete"),
    # 中文危险命令
    ("删除", "delete"),
    ("删除文件", "delete file"),
    ...
    ("删除所有", "delete all"),
    ("删除全部", "delete all"),
    ("删除这一切", "delete everything"),
    ("把文件删除", "delete file"),
    ("把目录删除", "delete directory"),
    ("将文件删除", "delete file"),
    ("将目录删除", "delete directory"),
    ("执行危险操作", "dangerous operation"),
    ("rm -rf", "recursive delete"),
    # 葡萄牙语危险命令 (apagar=删除, deletar=删除, remover=移除, eliminar=删除)
    (r"\bapag[aeir]+\b", "delete"),
    (r"\bdelet[aeir]+\b", "delete"),
    (r"\bremov[aeir]+\b", "delete"),
    (r"\belimin[aeir]+\b", "delete"),
...
def _pp_is_dangerous_v3(text):
    """检测危险命令"""
    if not text:
        return False, ""
    for pattern, desc in PP_DANGEROUS_PATTERNS_V3:
        if re.search(pattern, text, re.IGNORECASE):
            return True, desc
    return False, ""

# 审批请求存储 (in-memory)
PP_PENDING_APPROVALS = {}

def _pp_add_pending_v3(approval_id, data):
    """添加待审批请求"""
    PP_PENDING_APPROVALS[approval_id] = data

def _pp_remove_pending_v3(approval_id):
    """获取并移除待审批请求"""
    return PP_PENDING_APPROVALS.pop(approval_id, None)

def _pp_build_approval_card_v3(approval_id, sender_name, sender_open_id, command, danger_type):
    """构建审批卡片"""
    card = {
        "msg_type": "interactive",
        "card": {
            "config": {"wide_screen_mode": True},
            "header": {
                "title": {"tag": "plain_text", "content": "\u26a0\ufe0f 危险命令待审批"},
                "template": "red"
            },
            "elements": [
                {
                    ...
                    "actions": [
                        {
                            ...
                            "type": "primary",
                            "value": {"pp_action": "pp_approve", "pp_approval_id": approval_id}
                        },
                        {
                            "tag": "button",
                            "text": {"tag": "plain_text", "content": "\u274c \u62d8\u7edd"},
                            "type": "danger",
                            "value": {"pp_action": "pp_deny", "pp_approval_id": approval_id}
                        }
                    ]
                },
                {
                    "tag": "note",
                    "elements": [
                        {"tag": "plain_text", "content": f"\u5ba1\u6279ID: {approval_id} | \u7b49\u5f85\u7ba1\u7406\u5458\u5904\u7406"}
                    ]
                }
            ]
        }
...
def _pp_notify_user_v3(adapter, chat_id, approved, command, approver_name):
    """通知用户审批结果"""
    try:
        if approved:
            content = (
        ...
        if loop:
            import asyncio
            asyncio.ensure_future(
                asyncio.ensure_future(adapter.send(chat_id=chat_id, content=content), loop=loop),
                loop=loop
            )
    except Exception as e:
        logger.warning(f"[PP] Notify user failed: {e}")



# PP_HANDLER_FIXED
def _pp_handle_card_action_v3(self, *, event: Any, action_value: dict, loop: Any) -> Any:
    """处理审批卡片的按钮点击"""
    try:
        pp_action = action_value.get("pp_action")
        pp_approval_id = action_value.get("pp_approval_id")

        if not pp_action or not pp_approval_id:
            return None

        # PP_ADMIN_CHECK_FIXED: 获取操作者信息并验证管理员权限
        pp_operator = getattr(event, "operator", None)

        # 尝试多种方式获取操作者ID
        pp_open_id = ""
        if pp_operator:
            # 方法1: open_id 属性
            pp_open_id = str(getattr(pp_operator, "open_id", "") or "")
            # 方法2: id 属性
            if not pp_open_id:
                pp_open_id = str(getattr(pp_operator, "id", "") or "")
            # 方法3: user_id 属性
            if not pp_open_id:
                pp_open_id = str(getattr(pp_operator, "user_id", "") or "")
            # 方法4: 直接遍历属性找 uid 或 openid
            if not pp_open_id:
                for attr in dir(pp_operator):
                    if 'open' in attr.lower() or attr == 'uid':
                        val = getattr(pp_operator, attr, None)
                        if val and isinstance(val, str):
                            pp_open_id = str(val)
                            break

        # 详细日志记录操作者信息
        logger.warning(f"[PP_V3] Card action: pp_action={pp_action}, pp_approval_id={pp_approval_id}")
        logger.warning(f"[PP_V3] Operator info: operator={pp_operator}, type={type(pp_operator)}")
        logger.warning(f"[PP_V3] Operator open_id extracted: '{pp_open_id}'")

        # 检查是否为管理员
        if not pp_open_id:
            logger.error("[PP_V3] Cannot identify operator - rejecting")
            return None

        if not _pp_is_admin_v3(pp_open_id):
            logger.error(f"[PP_V3] PERMISSION DENIED: Non-admin {pp_open_id} tried to {pp_action}")
            # 记录到审计日志
            _pp_log_v3("UNAUTHORIZED", pp_open_id, "Unknown", f"approval_id={pp_approval_id}", "REJECTED", f"action={pp_action}")
            return None

        logger.warning(f"[PP_V3] Admin {pp_open_id} authorized to {pp_action}")

        # 获取待审批请求
        pp_pending = _pp_remove_pending_v3(pp_approval_id)
        if not pp_pending:
            logger.warning(f"[PP_V3] Approval {pp_approval_id} not found or already resolved")
            return None

        pp_user_chat_id = pp_pending.get("chat_id", "")
        pp_command = pp_pending.get("command", "")
        pp_user_name = pp_pending.get("user_name", "")

        # 获取审批人名称
        pp_approver_name = "Admin"
        try:
            pp_approver_name = self._get_cached_sender_name(pp_open_id) or pp_approver_name
        except Exception:
            pass

        # 处理批准/拒绝
        if pp_action == "pp_approve":
            _pp_notify_user_v3(self, pp_user_chat_id, True, pp_command, pp_approver_name)
            _pp_log_v3("APPROVED", pp_pending.get("user_open_id", ""), pp_user_name, pp_command, "APPROVED", f"by={pp_open_id}")
            logger.info(f"[PP_V3] Approval {pp_approval_id} approved by {pp_open_id}")
        else:
            _pp_notify_user_v3(self, pp_user_chat_id, False, pp_command, pp_approver_name)
            _pp_log_v3("DENIED", pp_pending.get("user_open_id", ""), pp_user_name, pp_command, "DENIED", f"by={pp_open_id}")
            logger.info(f"[PP_V3] Approval {pp_approval_id} denied by {pp_open_id}")

        return None
    except Exception as e:
        logger.error(f"[PP_V3] Card action error: {e}")
        return None
