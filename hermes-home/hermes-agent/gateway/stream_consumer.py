...
class StreamConsumerConfig:
    ...
    cursor: str = " ▉"
    output_mode: str = "full"  # "full" | "minimal" - minimal filters intermediate status msgs
...
class GatewayStreamConsumer:
    ...
    def __init__(
        ...
        self._accumulated = ""
        self._message_id: Optional[str] = reply_to_id
        ...
        # Minimal output mode: filter intermediate status lines
        self._minimal_mode = config.output_mode == "minimal" if config else False
        self._minimal_line_buf = ""  # Buffer for incomplete lines in minimal mode
            ...
            if self._in_think_block:
                ...
                if best_len:
                    ...
                    if held_back:
                        ...
                        # Minimal mode: filter status lines before accumulating
                        if self._minimal_mode:
                            buf = self._minimal_filter(buf)
                        self._accumulated += buf
                    return

    def _minimal_filter(self, text: str) -> str:
        if not self._minimal_mode or not text:
            return text
        self._minimal_line_buf += text
        lines = self._minimal_line_buf.split('\n')
        incomplete = lines[-1]
        ...
        result = []
        for line in lines:
            if not self._is_status_line(line):
                result.append(line)
        self._minimal_line_buf = incomplete
        joined = '\n'.join(result)
        if incomplete and self._is_status_line(incomplete):
            incomplete = ''
        return joined + ('\n' if text.endswith('\n') else '')
    ...
    def _is_status_line(self, line: str) -> bool:
        ...
        if not line:
            return False
        # Tool call output patterns: filter artifact labels
        if any(line.startswith(prefix) for prefix in (
            "💻", "📚", "🔎", "🌐", "📊", "📖", "🔧", "🎯", "✅", "❌", "⚠️", "🔍",
            "📚 skill_view:", "🔎 search_files:", "💻 terminal:",
            "🌐 browser_navigate:", "📊 data:", "📖 docs:",
        )):
            return True
        # Comment lines that look like tool calls
        if line.startswith("# ") and any(kw in line for kw in (
            "curl", "search", "try", "check", "get ", "fetch", "http",
        )):
            return True
        # Pure status lines (no domain content)
        _patterns = (
            (r"^(正在|开始|完成|结束|成功|失败|错误|抱歉)$", None),
            (r"^Step\s*\d+", None),
            (r"^\d+\.\s", None),
            (r"^完成[了！]?$", None),
            (r"^成功[了！]?$", None),
        ...
        for pattern, _ in _patterns:
            if _re.match(pattern, line):
                return True
        if len(line) < 12 and any(c in line for c in '...！！？？'):
            return True
        return False
            ...
            while True:
                ...
                if got_done:
                    ...
                    if self._accumulated:
                        if self._minimal_mode:
                            # Minimal mode: send first line as acknowledgment, then filtered results
                            lines = self._accumulated.split(chr(10))
                            first_line = lines[0] if lines else ""
                            rest_lines = lines[1:] if len(lines) > 1 else []
                            # Send first line (acknowledgment) as-is
                            if first_line.strip():
                            ...
                            # Filter and send rest (tool calls + status lines removed)
                            if rest_lines:
                                filtered_lines = [l for l in rest_lines if not self._is_status_line(l)]
                                # Also filter incomplete line buffer content
                                filtered_text = chr(10).join(filtered_lines)
    ...
    async def _send_new_chunk(self, text: str, reply_to_id: Optional[str]) -> Optional[str]:
        ...
        try:
            ...
            if result.success and result.message_id:
                ...
                self._edit_supported = bool(result.message_id)
            elif result.success and not result.message_id:
                self._edit_supported = False
                self._already_sent = True
            return self._message_id
        except Exception as e:
            logger.error("Error sending new chunk: %s", e)
            self._edit_supported = False
            return None
    ...
    async def _send_fallback_final(self, text: str) -> None:
        ...
        """
        # Minimal mode: buffer handled by got_done path
        if self._minimal_mode:
            return
    ...
    async def _send_commentary(self, text: str) -> bool:
        ...
        # Minimal mode: suppress all commentary (tool call output)
        if self._minimal_mode:
            return True
        text = self._clean_for_display(text)
        if not text.strip():
    ...
    async def _send_or_edit(self, text: str) -> bool:
        ...
        """
        # Minimal mode: buffer everything, suppress intermediate sends
        if self._minimal_mode:
            return True
